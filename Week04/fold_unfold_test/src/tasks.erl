...
%%
c_foldr(List, Accum, Fold_fun) when not is_list(List) -> not_list;
c_foldr([], Accum, Fold_fun) -> Accum;
...
%%
%% Parameters - 1)A list of any type of elements that is to be folded into 1 value
%%              2)An initia lvalue to which is merged the elements of the list
%%				3)A lambda function used to merge the accumulator and each element of the list
%% Value - A single value of some type
%% Complexity - O(n*u) where u is the complexity of the fold-func lambda function
%%
c_foldl(List, Acumme, Fold_fun) when not is_list(List)-> not_list;
c_foldl([], Accume, Fold_fun) -> Accume;
...
%%
c_unfold(Count, State, Unfold_fun) when not is_number(Count) -> not_number;
c_unfold(0, State, Unfold_fun) -> [];
...
%%% @end
%%%-------------------------------------------------------------------
generate_primes(Limit) when not is_number(Limit) -> not_number;
generate_primes(0) -> [3,2];
generate_primes(Limit) when Limit < 1 -> [];
% generate_primes(Limit)->
% 	Primes = [(6 * Limit - 1) | generate_primes(Limit-1)],
% 	Primes = [(6 * Limit + 1) | Primes],
% 	[PotentialPrime || PotentialPrime <- Primes, isPrime(PotentialPrime) == true].

generate_primes(Limit) ->
	Primes = [],
	if
		isPrime(6*Limit-1) == true->
			Primes = [(6*Limit-1)|Primes];
		isPrime(6*Limit+1) == true->
			Primes = [(6*Limit+1)|Primes]
	end,
	[Primes|generate_primes(Limit-1)].
